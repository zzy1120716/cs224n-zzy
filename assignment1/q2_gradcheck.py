...
def your_sanity_checks():
    ...
    log = lambda x: (np.sum(np.log(x)), 1 / x)

    print("Running your sanity checks...")
    gradcheck_naive(exp, np.array(123.456))
    gradcheck_naive(exp, np.random.randn(3,))
    gradcheck_naive(exp, np.random.randn(4, 5))
    gradcheck_naive(log, np.array(123.456))
    gradcheck_naive(log, np.abs(np.random.randn(3,)))
    gradcheck_naive(log, np.abs(np.random.randn(4, 5)))
    print("")
